...
def group_card(g):
    ...
    return f'<article class="product-card featured-group"><a class="product-image" href="{escape(url)}"{external}><img src="{escape(image)}" alt="{escape(title)}" width="480" height="480" loading="lazy"></a><div class="product-body"><p class="product-brand">Más vendidos</p><h3><a href="{escape(url)}"{external}>{escape(title)}</a></h3><a class="secondary-button" href="{escape(url)}"{external}>{'Ver opciones' if query else 'Consultar al asesor'}</a></div></article>'
