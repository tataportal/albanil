...
(()=>{
 ...
 const res=await fetch(api,{method:'POST',headers:{'Content-Type':'application/json','Idempotency-Key':key},body}),data=await res.json();if(!res.ok)throw Error(data.error||'No se pudo registrar. Inténtalo nuevamente.');receipt=data.complaint;form.hidden=true;document.querySelector('#complaint-success').hidden=false;document.querySelector('#receipt-number').textContent=receipt.reference;document.querySelector('#receipt-content').innerHTML=AlbanilComplaintReceipt.html(receipt);document.querySelector('#receipt-heading').setAttribute('tabindex','-1');document.querySelector('#receipt-heading').focus();
