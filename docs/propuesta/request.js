...
window.createAlbanilRequest = function ({getItems, service, notify, escape:esc}) {
 ...
 function render(){
  ...
  $('#request-materials').innerHTML=items.map(i=>`<li><div><strong>${esc(i.title)}</strong><small>${i.productId?`Ref. ${i.productId}`:'Pendiente de identificar'}${i.original?` · ${esc(i.original)}`:''}</small></div><span>${esc(i.quantity)} ${esc(i.unit||'unidad no indicada')}</span></li>`).join('')||'<li>No has agregado materiales.</li>';
 ...
 $('#request-download').addEventListener('click',()=>{
  ...
  const lines=['ALBAÑIL | SOLICITUD DE COTIZACIÓN','Documento preparado por el cliente. No enviado ni registrado en la tienda.',`Fecha: ${new Date().toLocaleString('es-PE')}`,'',`Cliente: ${c.name}`,`WhatsApp / teléfono: ${c.phone}`,`Correo: ${c.email||'No indicado'}`,`Empresa: ${c.company||'No indicada'}`,`RUC: ${c.ruc||'No indicado'}`,`Modalidad: ${c.delivery==='retiro'?'Retiro en tienda':'Entrega'}`,`Destino: ${c.destination||'Retiro en tienda'}`,'','MATERIALES',...p.items.map((i,n)=>`${n+1}. ${i.quantity} ${i.unit||'(unidad no indicada)'} × ${i.query}\n   ${i.productId?'Ref. '+i.productId:'PENDIENTE DE IDENTIFICAR'}${i.original?'\n   Texto original: '+i.original:''}`),'',`Observaciones: ${c.notes||'Sin observaciones'}`,`Flete: ${c.delivery==='retiro'?'No aplica por retiro':'Por calcular según destino y cantidad'}`];
