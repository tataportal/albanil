...
(() => {
 ...
 async function prepare(){
  ...
  $('#intake-share').hidden=!(navigator.share&&navigator.canShare);$('#intake-delivery-status').textContent='Todavía no se ha enviado a la tienda.';
 ...
 async function bundle(){if(!packet)throw Error('Prepara la solicitud primero.');const zip=await script('assets/vendor/fflate.min.js','fflate');const utf=s=>new TextEncoder().encode(s);const content=['ALBAÑIL | SOLICITUD DE COTIZACIÓN',packet.reference,'No constituye cotización ni reserva. Confirmar cantidades, unidades, precios y flete.','',...packet.rows.map((r,i)=>`${i+1}. ${r.quantity||'CANTIDAD POR CONFIRMAR'} ${r.unit||'(unidad por confirmar)'} | ${r.material}\n   ${r.status}${r.suggestions?.length?'\n   Sugerencias: '+r.suggestions.map(p=>p.title+' [Ref. '+p.id+']').join('; '):''}\n   Origen: ${r.source}${r.original?'\n   Original: '+r.original:''}`),'','ARCHIVOS ORIGINALES',...packet.files.map(f=>`${f.name}: ${f.status}`),'','Flete: por confirmar según destino.'].join('\n');const entries={'solicitud.txt':utf(content),'solicitud.json':utf(JSON.stringify({reference:packet.reference,rows:packet.rows,attachments:packet.files.map(({name,status})=>({name,status}))},null,2))};if(packet.raw)entries['texto-original.txt']=utf(packet.raw);for(let i=0;i<packet.files.length;i++){const f=packet.files[i];entries['originales/'+(i+1)+'-'+f.name.replace(/[^a-zA-Z0-9._-]/g,'_')]=new Uint8Array(await f.blob.arrayBuffer());}return new File([zip.zipSync(entries,{level:0})],packet.reference+'.zip',{type:'application/zip'});}
 $('#intake-download').addEventListener('click',async()=>{try{const file=await bundle(),url=URL.createObjectURL(file),a=document.createElement('a');a.href=url;a.download=file.name;a.click();setTimeout(()=>URL.revokeObjectURL(url),30000);$('#intake-delivery-status').textContent='Paquete descargado. Abre WhatsApp y adjunta el ZIP; todavía no se ha enviado.';}catch(e){$('#intake-delivery-status').textContent=e.message;}});
 $('#intake-share').addEventListener('click',async()=>{try{const f=await bundle();if(!navigator.canShare({files:[f]}))throw Error('Descarga el paquete y adjúntalo en WhatsApp.');await navigator.share({files:[f],title:'Solicitud Albañil',text:packet.reference});$('#intake-delivery-status').textContent='Paquete compartido mediante tu dispositivo. Confirma en WhatsApp que lo enviaste al asesor.';}catch(e){$('#intake-delivery-status').textContent=e.name==='AbortError'?'Envío cancelado. Tu solicitud sigue aquí.':e.message;}});
 $('#intake-whatsapp').addEventListener('click',e=>{e.currentTarget.href='https://wa.me/51968406042?text='+encodeURIComponent(`Hola, preparé la solicitud ${packet.reference} en la web (${packet.rows.length} renglones y ${packet.files.length} archivos). Adjuntaré el paquete ZIP con el detalle y los originales para que lo revise un asesor.`);});
})();
