(function(root){
 ...
 root.createAlbanilListBuilder=function({products,addProduct,getSummary,escape,notify}){
  ...
  $('#paste-form').addEventListener('submit',event=>event.preventDefault());
    ...
    let searchTimer;
    $('#builder-search').addEventListener('input',()=>{clearTimeout(searchTimer);searchTimer=setTimeout(searchManual,120);});
    $('#builder-search-form').addEventListener('submit',event=>{event.preventDefault();clearTimeout(searchTimer);searchManual();});
