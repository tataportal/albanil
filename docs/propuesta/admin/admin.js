...
(() => {
 ...
 if(demo){
  ...
  fetch('../catalog.json').then(r=>r.json()).then(catalog=>{requests=[];products=catalog.products.map(p=>({...p,sku:'',unit:p.unit||'',stock:null,availability:'Por confirmar',currency:'PEN',tax:'confirmar',version:0}));exchangeRate={rate:null,version:0,updated_at:null};
    try{const saved=JSON.parse(localStorage.getItem(AlbanilPricing.demoKey));if(saved&&AlbanilPricing.validRate(saved.rate)){exchangeRate={rate:saved.rate,version:0,updated_at:saved.updated_at};const savedProducts=new Map((saved.products||[]).map(p=>[p.id,p]));products=products.map(p=>({...p,...(savedProducts.get(p.id)||{})}));}}catch{}
