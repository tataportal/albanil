...
(() => {
 ...
 async function refresh(){const [p,r,fx]=await Promise.all([api('products'),api('requests'),api('exchange-rate')]);products=p.products;requests=r.requests;exchangeRate=fx;$('#new-product').hidden=!(await api('health').catch(()=>({}))).productCreation;renderProducts();renderRequests();renderFX();}
 async function enter(data){token=data.token||'';csrf=data.csrf||'';await refresh();$('#login-view').hidden=true;$('#workspace').hidden=false;$('#logout').hidden=false;message('');}
