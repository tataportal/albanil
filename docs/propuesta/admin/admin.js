...
(() => {
 ...
 async function refresh(){
  ...
  renderProducts();renderRequests();renderFX();await AlbanilComplaintsAdmin.refresh(api,currentUser);await AlbanilBannersAdmin.refresh(api,currentUser);
 }
 ...
 const sections={banners:{title:'Banners',hash:'banners'},requests:{title:'Solicitudes',hash:'solicitudes'},products:{title:'Productos y stock',hash:'productos'},exchange:{title:'Tipo de cambio',hash:'tipo-cambio'},complaints:{title:'Libro de reclamaciones',hash:'reclamos'}};
 function showSection(){
  const selected=Object.keys(sections).find(key=>permitted(key)&&sections[key].hash===location.hash.slice(1))||['requests','products','exchange','complaints','banners'].find(permitted)||'requests';
 ...
 const f=$('#request-form').elements;f.status.value=r.status;f.retentionClass.value=r.retentionClass||'unclassified';f.lastCustomerContact.value=r.lastCustomerContact||'';f.lastCustomerContact.min=r.created_at.slice(0,10);f.lastCustomerContact.max=new Date().toISOString().slice(0,10);f.agent.value=r.agent;f.notes.value=r.notes;$('#request-error').textContent='';$('#request-dialog').showModal();});
