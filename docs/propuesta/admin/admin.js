...
(() => {
 ...
 function renderProducts(){
  ...
  $('#product-rows').innerHTML=found.slice(0,limit).map(p=>`<tr><td><div class="table-product"><img src="../${esc(p.image)}" alt="" width="50" height="50"><div><strong>${esc(p.title)}</strong><small>${esc(p.brand)} · ${p.sku?`SKU ${esc(p.sku)}`:`Ref. ${esc(p.reference||p.id)}`}</small></div></div></td><td>${money(p)}${p.currency==='USD'?`<small>En la web: ${AlbanilPricing.label(AlbanilPricing.apply(p,exchangeRate.rate))}</small>`:''}<small>${esc(p.unit||'Unidad no registrada')}</small></td><td>${p.stock===null?'—':esc(p.stock)}</td><td><span class="availability" data-state="${esc(p.availability)}">${esc(p.availability==='Por confirmar'?'Stock sin registrar':p.availability)}</span></td><td><button class="secondary-button" data-edit="${p.id}" aria-label="Editar ${esc(p.title)}">Editar</button></td></tr>`).join('')||'<tr><td colspan="5">No encontramos productos con esos filtros.</td></tr>';
  $('#more-products').hidden=limit>=found.length;$('#more-products').textContent=`Ver ${Math.min(30,found.length-limit)} más`;
 }
 function savePreview(){
  if(!demo)return;
  try{localStorage.setItem(AlbanilPricing.demoKey,JSON.stringify({rate:exchangeRate.rate,updated_at:exchangeRate.updated_at,products:products.filter(p=>p.price!=null).map(p=>({id:p.id,price:p.price,currency:p.currency,unit:p.unit,tax:p.tax}))}));}catch{throw Error('El navegador no permite guardar los cambios.');}
 }
 function renderFX(){
 ...
 $('#fx-form').addEventListener('submit',async e=>{
  ...
  try{if(!AlbanilPricing.validRate($('#fx-rate').value))throw Error('Ingresa un tipo de cambio positivo con máximo 4 decimales.');
   ...
   if(demo)savePreview();renderFX();renderProducts();message(demo?'Tipo de cambio aplicado en este navegador. Abre «Ver precios en la web».':'Tipo de cambio guardado. La web lo usará al cargar los precios.');
 ...
 async function refresh(){const [p,r,fx]=await Promise.all([api('products'),api('requests'),api('exchange-rate')]);products=p.products;requests=r.requests.filter(r=>!r.customer.name.endsWith('· DEMO'));exchangeRate=fx;renderProducts();renderRequests();renderFX();}
 ...
 $('#edit-form').addEventListener('submit',async e=>{e.preventDefault();const b=e.target.querySelector('button[type=submit]');b.disabled=true;const values=Object.fromEntries(new FormData(e.target));try{const updated=await api(`products/${editing.id}`,{method:'PATCH',body:JSON.stringify({...values,version:editing.version})});products=products.map(p=>p.id===updated.id?updated:p);renderProducts();previewFX();if(demo)savePreview();$('#edit-dialog').close();message(demo?'Cambios aplicados en este navegador.':'Precio y stock guardados. El catálogo los leerá al cargar la página.');}catch(err){$('#edit-error').textContent=err.message;}finally{b.disabled=false;}});
 ...
 $('#request-form').addEventListener('submit',async e=>{e.preventDefault();const button=e.target.querySelector('button');button.disabled=true;try{const r=await api(`requests/${requestEditing.reference}`,{method:'PATCH',body:JSON.stringify({...Object.fromEntries(new FormData(e.target)),version:requestEditing.version})});requests=requests.map(old=>old.reference===r.reference?r:old);renderRequests();$('#request-dialog').close();message(demo?'Seguimiento actualizado en este navegador.':'Seguimiento guardado. El stock no se modificó.');}catch(err){$('#request-error').textContent=err.message;}finally{button.disabled=false;}});
 ...
 if(demo){
  ...
  $('#fx-form button').textContent='Aplicar en este navegador';$('#edit-form button[type=submit]').textContent='Aplicar en este navegador';
  fetch('../catalog.json').then(r=>r.json()).then(async catalog=>{requests=[];products=catalog.products.map(p=>({...p,sku:'',unit:p.unit||'',stock:p.stock??null,availability:p.availability||'Por confirmar',currency:p.currency||'PEN',tax:p.tax||'confirmar',version:0}));exchangeRate={...catalog.exchangeRate,version:0};
    try{const saved=JSON.parse(localStorage.getItem(AlbanilPricing.demoKey));if(saved&&AlbanilPricing.validRate(saved.rate)){exchangeRate={rate:saved.rate,version:0,updated_at:saved.updated_at};const savedProducts=new Map((saved.products||[]).map(p=>[p.id,p]));products=products.map(p=>({...p,...(!p.dataUpdatedAt||new Date(saved.updated_at)>=new Date(p.dataUpdatedAt)?savedProducts.get(p.id)||{}:{})}));}}catch{}
    if(window.AlbanilSettings)exchangeRate=await AlbanilSettings.publicRate(catalog.exchangeRate);
    renderProducts();renderRequests();renderFX();$('#request-count').textContent='Conecta el servicio de solicitudes para recibir pedidos aquí.';
  }).catch(()=>message('No se pudo cargar el catálogo. Recarga la página.'));
  return;
 }
